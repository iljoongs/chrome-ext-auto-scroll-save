...
3. **HTML 파일 다운로드**:
   ...
   - `chrome.downloads.download({ url: dataUrl, filename: "${safeTitle}.html", saveAs: true })`
     로 저장 대화상자 표시 (사용자가 저장 위치를 고를 수 있는 유일한 다운로드)

> **알려진 제약사항 (README에 명시할 것)**: 크롬 다운로드 API는 리소스 폴더를
> 사용자가 대화상자에서 고른 임의의 위치에 맞춰 저장해주지 않는다. 리소스
> 파일들은 기본 다운로드 폴더 기준 상대 경로(`${safeTitle}_files/...`)로
> 저장되고, HTML 파일만 `saveAs: true`로 위치를 고를 수 있다. 따라서 사용자가
> 기본 다운로드 폴더가 아닌 다른 위치를 선택하면 HTML 파일과 리소스 폴더가
> 서로 다른 위치에 놓일 수 있다. 이 문제를 피하려면:
> - (권장) HTML도 `saveAs: false`로 두고 항상 기본 다운로드 폴더에 저장해
>   `_files` 폴더와 항상 함께 있도록 하거나,
> - 사용자에게 "저장 대화상자에서 기본 다운로드 폴더를 선택해주세요"라고
>   안내 문구를 남길 것
> 둘 중 하나를 선택하고 그 이유를 README에 설명할 것 (기본값은 전자를 권장).
...
- 저장 결과물이 `<제목>.html` + `<제목>_files/` 폴더 형태라는 점,
  둘을 항상 같은 위치에 함께 두어야 이미지가 정상적으로 보인다는 점
- 위 "알려진 제약사항"에서 결정한 저장 위치 처리 방식 안내
